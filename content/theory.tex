...
In our work, the Kalman filter provides the state estimates needed by the MPC controller. For the Koopman-based MPC, the KF runs in the lifted state space to estimate the Koopman state vector used for prediction and control. In contrast, for the linear MPC, the KF estimates the internal state of the first-order linear model that describes the main thermal dynamics. Using the same estimation structure for both methods ensures a fair and consistent basis for comparison.
