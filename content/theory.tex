...
This chapter provides the theoretical foundation for the modeling and control methods employed in this thesis.
...
The model follows an encoder--linear dynamics--decoder structure~\cite{goodfellow2016deep}. The encoder maps the measured output to a lifted state
\begin{equation}
z_k = f_y(y_k),
\label{eq:koopman_encoder}
\end{equation}
where $f_y(\cdot)$ is a trainable nonlinear mapping. In the implemented model, the encoder is a fully connected neural network with three hidden layers of 60 neurons each and maps the measured output $y_k$ to a lifted state $z_k \in \mathbb{R}^{10}$.
...
MPC operates according to the receding horizon principle. Although an optimal input sequence is computed over the prediction horizon, only the first control input is applied to the system \cite{martin2019mpc}. At the next sampling instant, the optimization problem is solved again using updated state estimates.
...
Input and output constraints reflect physical and operational limitations of the controlled system. In thermal processes, input constraints typically represent actuator saturation, while output constraints are used to keep temperatures within safe operating limits \cite{rawlings_mpc, martin2019mpc}.
...
Koopman-based Model Predictive Control (Koopman MPC) combines the standard first-order MPC framework with a data-driven Koopman model and a Kalman filter for state estimation. 
In contrast to the linear MPC based on a first-order model, the prediction model in Koopman MPC is formulated in the lifted state space introduced in Section~\ref{sec:deep_koopman}. 
...
The same quadratic cost function and input and output constraints as in first-order MPC are employed.
