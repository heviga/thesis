...
This chapter presents the theoretical foundation for the modeling and control methods employed in this thesis.
...
\end{equation}
where $f_y(\cdot)$ is a trainable nonlinear mapping. The encoder in our constructed model maps the measured output $y_k$ to a lifted state $z_k \in \mathbb{R}^{10}$ using a fully connected neural network with three hidden layers of 60 neurons each.
...
All parts of the model, including the encoder, the lifted linear dynamics, and the decoder, are identified simultaneously from measured trajectories. The identification is performed using the \texttt{Neuromancer} framework~\cite{neuromancer2022}, which formulates the training as a structured optimization problem and enables gradient-based learning.
...
The state estimates that MPC controller requires in our work are obtained using the Kalman filter. To estimate the Koopman state vector utilized for prediction and control in the Koopman-based MPC, the KF operates in the lifted state space.

For the MPC derived from the first-order step-response, the KF estimates the internal state that describes the main thermal dynamics. Using the same estimation structure in both approaches provides a consistent basis for comparison.
...
For the simple linear MPC, a first-order representation obtained from the step response was used for prediction. The continuous-time transfer function can be written as
\begin{equation}
G(s) = \frac{K}{\tau s + 1},
...
Koopman-based Model Predictive Control (Koopman MPC) integrates the conventional first-order MPC approach with a data-driven Koopman model and uses a Kalman filter for state estimation. Koopman MPC formulates its prediction model in the lifted state space described in Section~\ref{sec:deep_koopman}. The lifted-state dynamics and output mapping from equations~\eqref{eq:koopman_state} and~\eqref{eq:koopman_output} are incorporated directly into the MPC prediction process \cite{korda2018,koopman_mpc_review}.
