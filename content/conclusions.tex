...
\label{eq:rmse}
\end{equation}
The RMSE was computed in physical units, i.e., in degrees Celsius, so that the prediction error can be directly interpreted with respect to the measured outlet temperature.
Since this metric is evaluated for open-loop prediction, it is denoted in the following as $\mathrm{RMSE}_{\mathrm{OL}}$. For the identified first-order linear model, the obtained value on the test data was
\begin{equation}
...
The first-order MPC behaves more conservatively, leading to slightly lower RMSE but slower input adjustment.
